...
# Overview
The Arduino UNO Q (ABX00162) serves as the control hub for our in-house developed Space-Time Coding Metasurface (STCM). The high-level computational power of its Qualcomm® Dragonwing™ QRB2210 helps determine the spatial phase gradient for beam steering and the time-domain coding sequence for digital modulation in real time. Its STMicroelectronics® STM32U585  (Arm® Cortex®-M33) microcontroller obtains the spatial phase information and the time-domain coding sequence and channels them to 16 columns of the RIS board to enable the required switching of PIN diodes. The two domains communicate via the Arduino Bridge, a software-based Remote Procedure Call (RPC) layer. Moreover, the system features an autonomous tracking mode where the beamsteering angle ($\theta_{des}$) is dynamically calculated using a trilateration algorithm. This allows the metasurface to track a mobile target based on RSSI (Received Signal Strength Indicator) data from external anchors.
# Project Structure
